    ...
    def __init__(self, annotation_dir_path ):
        ...
        for jsonFilePath in self.json_files:
            ...
            for index in range(len(currentAnnotation['shapes'])):
                ...
                annotationList.append((label, bbox[0] - left,bbox[1] - upper,bbox[2] - left,bbox[3] - upper))
            ...
            for i in range(row):
                for j in range(column):
                    ...
                    if count %3 == 0:
                        dirPath = os.path.join(dirPath,'output','test')
                    elif count %4 ==0:
                        dirPath = os.path.join(dirPath,'output','valid')
                    else:
                        dirPath = os.path.join(dirPath,'output','train')
                    ...
                    with open(txtPath, 'w') as file:
                        for ann in annotationList:
                            ...
                            if ann[1] >= subLeft and ann[2] >= subUpper and ann[3] <=  subRight and ann[4] <= subLower:
                                ann[1] = ann[1] - subLeft
                                ann[2] = ann[2] - subUpper
                                ann[3] = ann[3] - subLeft
                                ann[4] = ann[4] - subUpper
                                contentPrepared = f'{self.classes.index(ann[0])} {((ann[3]+ann[1])/2)/subWidth} {((ann[4]+ann[2])/2)/subHeight} {(ann[3]-ann[1])/subWidth} {(ann[4]-ann[2])/subHeight}\n'
